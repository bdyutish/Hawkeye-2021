  ...
  return (
    <div className="admin">
      ...
      {leaderboard.map((user , ind) => {
       // console.log(user);
        return (
     <Leaderboard userid={user["_id"]} key={ind} index={(ind+1).toString()}/>
      ...
      <div className="region-item" key={ind}>
        ...
        <Link to={`/admin/${reg["_id"]}/`}><button className="button-admin">Modify</button></Link>
      </div>
...
function Leaderboard({  userid  , index}: Details): ReactElement { 
  ...
  return(
    <div className="row" >
            ...
            <div>
            {userdata.isBanned && <button type="button" className="btn-danger1" onClick={() => (banUser(userdata._id))}>
                Un Ban
              </button>}
              {!userdata.isBanned && <button type="button" className="btn-danger" onClick={() => (banUser(userdata._id))}>
                 Ban 
              </button>}
            </div>
          </div>
...
function UnlockHints ():ReactElement {
  const unlockHint = async (event: React.FormEvent<HTMLFormElement>): Promise<any>  => {
    ...
    <div className="regions-container">
        <div className="region-item">
         <form onSubmit= {unlockHint} >
           ...
           <div> <input onChange={(e) => setque(parseInt(e.target.value))}/> </div>
           <label>Hint Level [1-3]</label><div> <input onChange={(e) => setlev(parseInt(e.target.value))}/> </div>
